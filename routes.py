...
def buyShares():
   ...
   if form.is_submitted():
      ...
      urlCsvDaily = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=' + result["name"] + "&apikey=" + key1 + "&datatype=csv"
      companyInfo = pd.read_csv(urlCsvDaily)
      companyInfo.to_csv('static/data/daily_adjusted.csv')

      graph_table = pd.read_csv('static/data/daily_adjusted.csv', usecols=['timestamp', 'close', 'high'])
      data = [go.Scatter(
         x = graph_table['timestamp'],
         y = graph_table['close'],
      )]
      ...
      fig_monthly = go.Figure(data=data, layout = layout)
      fig_monthly.write_image("static/photo/graph_monthly.png")
      daily_price = pd.read_csv('static/data/daily_adjusted.csv', usecols=['timestamp', 'close'], nrows=1)
      write_timestamp_close = daily_price.to_csv('static/data/daily_price.csv')
      ...
      stock_CSV_URL = 'https://www.alphavantage.co/query?function=LISTING_STATUS&apikey=' + key3 + "&datatype=csv"
      companyInfo = pd.read_csv(stock_CSV_URL, usecols=['symbol', 'name', 'assetType'])
      ...
      if form.is_submitted():
         try:
            holdings = session.query(customer_db.Holdings).first()
         
            holdings.name = company_name
            holdings.value = daily_price_close
            holdings.gain = daily_price_close
      ...
      money_left_after_buy = balance.amount + daily_price_close         ## Show gains, show values, update to SQL table holdings, user should be able to buy multiple stocks and invest in whatever amount of money they like
                                                                        ## with button to enter in money amount use try and except value error to make sure person inputting number. Show alert if not
      return render_template('buy_shares.html', title= company_name + " Share Price", header=company_name + " Share Rate", 
                             ...
                             daily_price_close="{:.2f}".format(daily_price_close), moneyAmt="{:.2f}".format(balance.amount), money_left_after_buy="{:.2f}".format(money_left_after_buy),name = company_name, form=form)
